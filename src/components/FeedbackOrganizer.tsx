...
분류 기준:
- required: 클라이언트가 명확하게 변경을 요청한 항목 (예: "로고 크기 줄여주세요", "색상 바꿔주세요")
- optional: 제안·불만·막연한 표현 등 재확인이 필요한 항목 (예: "전반적으로 좀 더 세련되게")
- clarified: 모호한 피드백을 실무적으로 구체화 (예: "톤이 밝다" → "전체 채도를 낮추거나 어두운 계열 색상으로 변경 고려")
- 모든 값은 한국어로 작성
- clarified는 모호한 항목만 포함 (명확한 항목 제외), 없으면 빈 배열`;
