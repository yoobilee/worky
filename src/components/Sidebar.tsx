...
export default function Sidebar({ isOpen, onClose, aiStatus }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* ── 헤더 ── */}
      <div className={[
        "flex items-center border-b border-slate-200 dark:border-zinc-800 shrink-0",
        isCollapsed ? "flex-col gap-2 px-2 py-3" : "gap-3 px-4 py-4",
      ].join(" ")}>
        {/* 로고 */}
        <div
        ...
        <button
          ...
          className={[
            "p-1.5 rounded-lg text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors shrink-0 hidden lg:flex",
          ...
          if (isCollapsed) {
            return (
              <div key={item.href} className="relative group">
                ...
                {/* 툴팁 */}
                <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2.5 px-2.5 py-1.5 bg-slate-800 dark:bg-zinc-700 text-white text-xs font-medium rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">
                  {item.label}
                  <span className="absolute right-full top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-800 dark:border-r-zinc-700" />
                </div>
              </div>
            );
          }
      ...
      {/* ── 하단 ── */}
      <div className={[
        "shrink-0 border-t border-slate-200 dark:border-zinc-800",
        isCollapsed ? "px-2 py-3 flex flex-col items-center gap-2" : "px-4 py-4",
      ].join(" ")}>
        {/* 접힌 상태: 다크모드 토글 */}
        {isCollapsed && (
          <div className="relative group">
            <button
              onClick={toggleTheme}
              aria-label={theme === "dark" ? "라이트 모드" : "다크 모드"}
              className="flex items-center justify-center w-10 h-10 rounded-xl text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
            >
              {theme === "dark" ? <IconSun className="w-4 h-4" /> : <IconMoon className="w-4 h-4" />}
            </button>
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2.5 px-2.5 py-1.5 bg-slate-800 dark:bg-zinc-700 text-white text-xs font-medium rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">
