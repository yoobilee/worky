    ...
    <div className="space-y-3 max-w-5xl mx-auto w-full">
      ...
      {showHistory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowHistory(false)}>
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-2xl p-6 w-full max-w-lg mx-4 flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4 shrink-0">
              <h3 className="text-base font-semibold text-slate-800 dark:text-slate-100">정리 히스토리</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 min-h-0">
              {history.length === 0 ? (
                <p className="text-sm text-slate-400 dark:text-zinc-500 text-center py-8">아직 정리 기록이 없습니다</p>
              ) : (
                history.map(h => (
