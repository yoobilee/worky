...
### 테스트 코드 리뷰 원칙
- **새로 추가되는 테스트 파일/테스트 케이스**: Codex 리뷰를 거치는 것을 권장.
  테스트가 "실행된다"와 "제대로 검증한다"는 다르다 — 실제로 PR #107(Playwright
  스모크 테스트)에서 클린 체크아웃 시 브라우저 미설치로 실패, 하드코딩된
  언어 텍스트로 인한 불안정한 assertion, URL만 확인하고 실제 렌더링 실패를
  못 잡는 취약한 검증 등 3건의 실질적 결함을 리뷰가 잡아낸 바 있음
  (Issue #108, PR #107 라운드 2).
- **기존에 검증된 테스트 파일의 사소한 수정** (셀렉터 변경, 타임아웃 조정
  등): 리뷰 생략 가능. 다만 "사소한 수정"과 "실질적으로 새 테스트 추가"의
  경계가 diff만으로 항상 명확하진 않으므로, 아직 이 구분을 워크플로우
  자동화 로직으로 강제하지는 않음 — 향후 사례가 더 쌓이면 자동 트리거
  조건화 여부를 재검토.
- 이 원칙은 현재(2026-09-03) 판단 기준으로 기록만 해둔 상태이며, 워크플로우
  코드에는 아직 반영되지 않음.

### PR 범위 통제 원칙

- 하나의 PR은 시작할 때 정한 기능 흐름 하나만 다룬다.
- 작업 중 그 범위 밖의 문제를 발견하면(심각도 무관), 해당 PR에서 고치지 않고 별도 GitHub Issue로 등록한 뒤 원래 작업으로 복귀한다.
- 판단 기준: 발견된 문제가 "이번 PR의 커밋이 만들어졌기 때문에 새로 생긴 것"이면 같은 PR에서 고치고, "이 PR과 무관하게 원래 있었던 것"이면 범위 밖으로 분리한다.
- Codex/Claude Code가 작업 중 범위 밖 P1을 스스로 판단해 수정하지 않도록, 세션 시작 시 이 PR의 범위를 한 문장으로 명시한다.
- 실제 사례(PR #148): "할 일 완료 상태 E2E 추가"로 시작했으나 테스트 정리 로직 결함(#149, 범위 안 — PR이 만든 부작용) → 그 수정이 만든 service-role 키 노출(#150, 범위 안 — 직접적 부작용) → workflow 동시성 문제(#152/#153, 범위 밖 — 원래부터 있던 별개 설계 문제, 이슈만 만들고 별도 처리했어야 함)로 이어짐.
