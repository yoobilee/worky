...
- **로컬 하드코딩 데이터 → Supabase 마이그레이션**: 다른 포트폴리오 프로젝트(frontend-interview-prep)에서 질문 데이터를 JS 파일에 하드코딩해 관리하던 구조가 사용자 제보 기능 확장의 걸림돌이 되는 것을 발견했습니다. Supabase로 데이터를 이전하며 콘텐츠와 코드를 분리했고, 이 경험을 바탕으로 Worky에서도 데이터와 로직을 처음부터 분리된 구조로 설계했습니다.
