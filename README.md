...
[![로그인 없이 게스트로 체험하기](https://img.shields.io/badge/%F0%9F%91%A4_로그인_없이_게스트로_체험하기-22C55E?style=for-the-badge)](https://worky-ai.vercel.app)

로그인 화면에서 **"게스트로 체험하기"**를 선택하면 회원가입 없이 더미 데이터로 바로 둘러보실 수 있습니다.
(Google 로그인은 승인된 테스트 계정만 가능하니, 게스트 버튼을 이용해주세요.)
...
마케팅 회사에서 일하는 지인은 각각 다른 거래처를 맡은 영업사원들의 스프레드시트를 한데 모아 업무를 보조해야 했습니다. 영업사원마다 열 이름과 배치 등 정리 방식이 달라 자신의 스프레드시트로 옮겨 모아볼 수는 있어도, 필요한 정보를 다시 살피는 과정이 불편했습니다. 이 자료를 한곳에서 모아 보고 거래처별 정보를 시각적으로 더 편하게 확인할 수 있도록 Worky의 거래처 관리 화면과 업무 흐름을 구성했습니다.

여기에 신입사원 시절 직접 겪었던 어려움(업무 용어, 메모 정리, 메일 답장 등)을 더해 다른 기능도 확장했습니다. 지인이 Worky를 실제로 사용하며 편해졌다고 말했고, 다른 페이지도 사용하면서 아쉬운 점이나 필요한 기능을 알려주면 바로 반영해 왔습니다. 현재도 유지보수와 기능 추가를 이어가며 다양한 직군의 직장인이 함께 쓸 수 있는 생산성 서비스로 넓혀가고 있습니다.
...
| **AI** | Groq API, GPT-OSS 120B, react-markdown + remark-gfm |
| **인증 / DB** | Supabase (Auth + Database + Realtime), Google OAuth, Gmail API |
| **배포** | Vercel |

> **왜 Groq인가?**
> 무료 티어가 충분하고, 한국에서 제약 없이 사용 가능하며, 동급 모델 대비 응답 속도가 매우 빠릅니다.
> 초기에는 LLaMA 4 Scout를 사용했으나, 2026년 6월 Groq의 모델 지원 종료에 따라 GPT-OSS 120B로 전환했습니다. 이후 실제 후보 모델 여러 개를 직접 비교 테스트해 응답 속도·정확도·다국어 안정성을 확인했고, 그 결과를 바탕으로 `reasoning_effort` 파라미터를 낮게 설정해 숨은 추론 과정에 토큰이 소모되어 응답이 잘리는 문제를 해결했습니다.
...
신입사원 업무 맥락을 고려한 답변과 대화 히스토리 유지를 지원하며, 답변은 마크다운(표 포함)으로 렌더링됩니다. 질문 내용에 따라 내가 등록한 거래처·일정·할 일 등 실제 데이터를 자동으로 반영해 답변합니다. 대화 시작 전에는 추천 질문을 먼저 보여드립니다.
...
PR이 열리면 Build Check와 E2E Check가 실행됩니다. [OpenAI Codex](https://openai.com/codex/) 리뷰가 도착하면 Claude Code 에이전트가 내용을 다시 읽어, 안전하게 고칠 수 있는 문제는 후속 커밋으로 처리하고 보안·설계처럼 판단이 필요한 사안은 Slack으로 사람 확인을 요청합니다. Codex가 현재 head를 검토한 뒤 지적 없이 PR 본문에 👍 반응을 남기고 필수 검사가 모두 성공하면 전용 워크플로우가 자동 병합합니다. 단, `.github/workflows/` 아래 파일을 변경하는 PR은 자동 병합하지 않고 사람 확인 후 수동 병합합니다. 리뷰가 오지 않거나 실행하지 못했다는 안내만 남긴 경우는 통과로 보지 않으며, 15분 안에 리뷰가 도착하지 않으면 별도 감시 워크플로우가 알려줍니다. 검사와 리뷰가 모두 확인된 PR만 병합합니다.

발견된 결함은 Worky의 이슈 정리 페이지와 같은 방식으로 GitHub Issue에도 등록되어, 제품 기능과 개발 과정을 같은 흐름으로 관리합니다.

<br/>

## 테스트 & 검증

기능 성격에 따라 단위 테스트, 브라우저 E2E, Supabase 보안 검증을 나눠 실행합니다.

| 구분 | 실행 명령 | 확인 내용 |
|---|---|---|
| 단위 테스트 | `npm run test:unit` | `src` 내부 로직과 UI 단위 검증 |
...
| 로그인 CRUD E2E | `npx playwright test --config=playwright.crud.config.ts` | 테스트 계정으로 일정·거래처 CRUD, 할 일 완료 유지 및 업무 메모 저장·수정·초기화를 검증 |
...
PR의 Build Check는 단위 테스트와 `npm run build`를 실행합니다. E2E Check는 게스트 흐름과 로그인한 테스트 계정의 일정·거래처 CRUD, 할 일 완료 유지 및 업무 메모 저장 흐름을 각각 실행합니다. 로그인 CRUD E2E는 사용자 access token을 REST 요청에 함께 전달하며, 테스트가 만든 데이터는 각 시나리오 종료 시 정리합니다. 할 일 테스트는 기존 항목이 자동 이월되지 않도록 브라우저 날짜를 기존 데이터보다 이전의 격리된 날짜로 고정하고, 테스트가 만든 항목만 배열에서 제거합니다(앱과 동일하게 빈 날짜 행은 유지). 메모 테스트는 실행 전 업무 메모 값을 종료 시 복원하고 회의·개인 메모가 바뀌지 않았는지 검증합니다. 일반 Playwright 설정은 테스트 계정 비밀값이 필요한 `*-crud.spec.ts`를 제외합니다.
...
- **리뷰 봇 교체 후 발견된 데이터 정합성 공백**: 리뷰 봇을 CodeRabbit에서 OpenAI Codex로 교체하는 과정에서, 결함이 발견되면 GitHub Issue를 자동 등록하고 동시에 Supabase에도 동일한 기록을 남기는 파이프라인의 일부를 수동으로(자동화를 거치지 않고 직접) 처리한 경우가 있었습니다. 이후 GitHub Issue 탭의 전체 개수와 앱 내 이슈 정리 페이지의 개수가 서로 달라진 것을 발견했고, 원인을 추적한 결과 "GitHub에만 등록하고 Supabase 기록은 건너뛴" 이슈 6건이 있었습니다. 두 시스템이 각자 독립적인 진실 소스(source of truth)를 갖는 구조에서는, 한쪽 파이프라인만 거치고 다른 쪽을 생략하면 시스템 두 개가 조용히 어긋난다는 걸 확인한 사례였습니다. 누락된 6건을 실제 GitHub 데이터 기준으로 Supabase에 백필(backfill)하고, 이후 두 시스템의 개수가 정확히 일치하는지 재검증하는 방식으로 정합성을 복구했습니다.
- **권한 강화 뒤 드러난 E2E 인증 누락**: Supabase 공개 권한을 최소화한 뒤 일정 CRUD E2E의 첫 조회가 401로 실패했습니다. 브라우저는 테스트 계정으로 로그인했지만, 테스트가 직접 보내는 REST 요청에는 anon key만 있고 사용자 access token이 없었던 것이 원인이었습니다. 로그인 세션에서 access token을 가져와 REST 요청에 함께 전달하도록 고쳐, 익명 접근을 허용하지 않으면서도 실제 로그인 사용자 흐름을 검증하도록 보완했습니다.
