...
PR이 열리면 Build Check와 E2E Check가 실행됩니다. [OpenAI Codex](https://openai.com/codex/) 리뷰가 도착하면 Claude Code 에이전트가 내용을 다시 읽어, 안전하게 고칠 수 있는 문제는 후속 커밋으로 처리하고 보안·설계처럼 판단이 필요한 사안은 Slack으로 사람 확인을 요청합니다. 리뷰가 오지 않거나 실행하지 못했다는 안내만 남긴 경우는 통과로 보지 않으며, 15분 안에 리뷰가 도착하지 않으면 별도 감시 워크플로우가 알려줍니다. 검사와 리뷰가 모두 확인된 PR만 병합합니다.
